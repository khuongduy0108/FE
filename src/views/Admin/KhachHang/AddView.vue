<template>
  <v-dialog v-model="dialog" max-width="650px">
    <v-card>
      <v-card-title class="headline">Thêm khách hàng mới</v-card-title>
      <v-card-text>
        <v-form>
          <v-container>
            <v-row>
      ...
      <v-card-actions>
        ...
        <v-btn color="primary" @click="addKhachHang">Lưu</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
...
export default {
  ...
  methods: {
    addKhachHang() {
      axios.post('https://localhost:44354/api/KhachHang', this.data)
        .then(response => {
          this.$emit('close');
